...
double* randomGauss(config_s config) {
	...
	z_comp[2] = 0. * config.stdv[2]  + config.mean[2];
...
void makeAxes(grid_s grid, config_s config, grid_s* ret_grid) {
	for (size_t dim_idx = 0; dim_idx < config.num_dim; dim_idx++) {
		if (grid.pnts[dim_idx] > 1) {
			...
			else { printf("Differing minumum and maxium detected for singular array, assuming midpoint.\n"); grid.min[dim_idx] += (grid.max[dim_idx] - grid.min[dim_idx]) / 2.0; }
		}
...
mag_s initMag(double* centre, double* dim, size_t* pnts, double* mag_mom, config_s config) {
	...
	for (size_t diple_idx = 0; diple_idx < mag.num_diples; diple_idx++) {
		for (size_t dim_idx = 0; dim_idx < config.num_dim; dim_idx++) {
			...
			mag.diple_moms[config.num_dim*diple_idx + dim_idx] = mag.mag_mom[config.num_dim*diple_idx + dim_idx];
		}
	}
...
int main()
{
	...
	printf("Complete.\n");
	/*
	printf("*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~* Printing Test Point *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~* \n");
	printf("Test Point:\n");
	printf("Poisition: %f, %f, %f. \n", c_grid.posit[0], c_grid.posit[1], c_grid.posit[2]);
	printf("B_Field: %.10e, %.10e, %.10e.\n", c_grid.mag_field[0], c_grid.mag_field[1], c_grid.mag_field[2]);
